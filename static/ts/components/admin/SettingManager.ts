...
import type { SettingManagerComponent, SettingManagerOptions, OpenAIModel } from '../../types/components.js';

declare const bootstrap: {
    Modal: new (element: HTMLElement) => { show: () => void; hide: () => void };
};

export class SettingManager implements SettingManagerComponent {
    private settingTable: SettingTable;
    private modal: { show: () => void; hide: () => void };
    private modalElement: HTMLElement;
    private options: SettingManagerOptions;

    constructor(options: SettingManagerOptions) {
    ...
    async editSetting(setting: Setting): Promise<void> {
        ...
        if (setting.category === 'OpenAI' && setting.key === 'model' && this.settingTable.availableModels.length > 0) {
            this.createModelSelect(editSettingValue as HTMLElement, setting);
        } else {
            await this.restoreInput(editSettingValue as HTMLElement, setting);
        }
    ...
    private createModelSelect(element: HTMLElement, setting: Setting): void {
    ...
    private async restoreInput(element: HTMLElement, setting: Setting): Promise<void> {
        ...
        const inputElement = document.getElementById('editSettingValue') as HTMLInputElement;
    ...
    private setFormValues(setting: Setting): void {
        const idElement = document.getElementById('editSettingId') as HTMLInputElement;
        const categoryElement = document.getElementById('editSettingCategory') as HTMLInputElement;
        const keyElement = document.getElementById('editSettingKey') as HTMLInputElement;
        
        if (idElement) idElement.value = setting.id.toString();
        if (categoryElement) categoryElement.value = setting.category;
        if (keyElement) {
    ...
    async saveChanges(): Promise<boolean> {
        const settingId = (document.getElementById('editSettingId') as HTMLInputElement)?.value;
        const settingValue = (document.getElementById('editSettingValue') as HTMLInputElement)?.value;
    ...
    private async handleDeleteAssistant(assistantId: string): Promise<void> {
        if (!assistantId) {
    ...
    private async handleFlushAssistantSettings(): Promise<void> {
    ...
    private async handleTestEmail(): Promise<void> {
        // Mostrar notificación de progreso
