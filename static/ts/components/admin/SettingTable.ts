...
import type { SettingTableComponent, SettingTableOptions, OpenAIModel } from '../../types/components.js';

declare const bootstrap: {
    Modal: new (element: HTMLElement) => { show: () => void; hide: () => void };
};

export class SettingTable implements SettingTableComponent {
    private container: HTMLElement;
    private confirmDialog: HTMLElement;
    private modal: { show: () => void; hide: () => void };
    public availableModels: OpenAIModel[] = [];
    private options: SettingTableOptions;

    constructor(containerId: string, options: SettingTableOptions = {}) {
    ...
    render(settings: Setting[]): void {
        ...
        Object.entries(groupedSettings).forEach(([category, categorySettings]) => {
            ...
            if (category === 'Email Configuration' && this.options.onTestEmail) {
                ...
                testButton.addEventListener('click', async () => {
                    try {
                        ...
                        await this.options.onTestEmail?.();
                    } finally {
            ...
            if (category === 'OpenAI' && this.options.onLoadModels) {
                ...
                loadButton.addEventListener('click', async () => {
                    try {
                        ...
                        await this.options.onLoadModels?.();
                    } finally {
            ...
            categorySettings.forEach(setting => {
                ...
                const isInformativeOnly = setting.category === 'OpenAI' && (
                    setting.key === 'assistant_id' ||
                ...
                if (this.options.onDelete && isAssistantId) {
                    const deleteButton = tr_element.querySelector('button[data-action="delete"]') as HTMLButtonElement;
                    if (deleteButton) {
                        deleteButton.addEventListener('click', (event) => {
                            ...
                            const handleDelete = async () => {
                                ...
                                try {
                                    await this.options.onDelete?.(setting.value);
                                    this.modal.hide();
                                    // Refresh settings
                            ...
                            document.getElementById('confirmAction')?.addEventListener('click', handleDelete);
                ...
                if (this.options.onFlush && isAssistantId) {
                    const flushButton = tr_element.querySelector('button[data-action="flush"]') as HTMLButtonElement;
                    if (flushButton) {
                        flushButton.addEventListener('click', (event) => {
                            ...
                            const newConfirmBtn = confirmBtn.cloneNode(true);
                            confirmBtn.parentNode?.replaceChild(newConfirmBtn, confirmBtn);
                            document.getElementById('confirmAction')?.addEventListener('click', handleFlush);
